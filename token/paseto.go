...
func NewPasetoMaker(symmetricKey string) (Maker, error) {
	if len(symmetricKey) != chacha20poly1305.KeySize {
		return nil, fmt.Errorf("invalid key size: %d must be exactly %d char", len(symmetricKey), chacha20poly1305.KeySize)
	}
...
func (m *PasetoMaker) VerifyToken(tokenString string) (*Payload, error) {
	...
	err := m.paseto.Decrypt(tokenString, m.symmetricKey, payload, nil)
	if err != nil {
